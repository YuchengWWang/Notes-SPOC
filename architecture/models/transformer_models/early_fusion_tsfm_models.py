...
class EarlyFusionCnnTransformer(nn.Module):
    ...
    def get_input_embedding_per_timestep(
        ...
        if "last_actions" in non_visual_sensors:
            last_actions_enc = self.last_actions_embed(non_visual_sensors["last_actions"]) # [2,50]->[2,50,512]这里用到一个nn.Embedding模块，这是一个权重可训练的词典，输入是某个索引输是这个item对应的embedding
            visual_feats = visual_feats + last_actions_enc # 这两个特征向量直接以相加的方式融合
        ...
        time_enc = self.time_encoder(time_ids) # [2,50] -> [2,50,512]
        visual_feats = visual_feats + time_enc # 还是以相加的形式融合

        return visual_feats, text_feats

    def decode_and_get_logits(self, embedded_features, text_feats, padding_mask=None):
        causal_mask = create_causal_mask(embedded_features.shape[1], embedded_features.device) #一个[50,50]的上三角矩阵*-inf
        decoder_output = self.decoder(
            tgt=embedded_features,  # [2,50,512]
            memory=text_feats,    # [2,64,512] memory是提供k,v的，tgt是提供q的，selfattn就是mha(tgt,tgt,tgt)，crossattn就是(tgt,memory,memory)
            tgt_mask=causal_mask,
            tgt_key_padding_mask=padding_mask,
        ) # [2,50,512]不管是crossattan还是selfattn维度都保持和q一样
        logits = dict(actions_logits=self.action_classifier(decoder_output)) #[2,50,512]->[2,50,20]
        return logits
    ...
    def forward(self, batch):
        ...
        logits = self.decode_and_get_logits(embedded_features, text_feats, padding_mask) #[2,50,512], [2,64,512], [2,50]
