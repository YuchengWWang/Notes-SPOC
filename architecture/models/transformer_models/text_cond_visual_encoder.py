...
class TextCondMultiCameraVisualEncoder(nn.Module):
    ...
    def encode_text(self, preproc_text_input):
        with torch.no_grad():
            if isinstance(self.text_encoder, TextTransformer):
                cls_feats, text_feats = self.text_encoder(preproc_text_input) # [2,64]的输入，输出[2,63,768]的text_feat局部特征和[2,768]的cls_feat全局特征。768是SigLIP模型定的
                text_feats = torch.cat([text_feats, cls_feats.unsqueeze(1)], dim=1) # 和图片一样，都有compressor或adapter把768压成512
            else:
    ...
    def encode_imgs(self, imgs):
        ...
        feats = feats.reshape(B * T, C_, H_ * W_).permute(0, 2, 1)  # [BT, 7*12, 512]
    ...
    def get_image_text_feats(self, frames, goals, text_feats):
        ...
        for sensor in frames.keys():
            ...
            all_img_features[sensor] = image_feats # [2*50, 7*12, 512]

        concatenated_feats = []

        for k in self.visual_sensors:
            corresponding_camera_token = getattr(self, f"visual_sensor_token_{k}") # [512]
        ...
        B, L, D = text_feats.shape # [2, 64, 512]
        text_feats_ = text_feats.unsqueeze(1).tile(1, T, 1, 1).reshape(B * T, L, D) # [100, 64, 512]
    ...
    def forward(
        ...
        input_features = [fusion_token, concatenated_feats, text_feats_] #[100,1,512],[100,168,512],[100,64,512]

        fused_feats = self.fusion_xformer(torch.cat(input_features, 1)) # [100,233,512]

        fused_feats = fused_feats[:, 0, :]  # [100,1,512] The output at the position of the [CLS] token serves as the goal-conditioned visual representatio
